...
function monthlyPayment() {
    if (!currentBank || availableBanks.value == 'choose') {
        ...
        if (P < 0) {
            downpaymentField.style.backgroundColor = 'rgb(225, 150, 150, 0.5)';
            document.querySelector("label[for='downpayment']").textContent = "Downpayment cannot be larger than initial loan";
            document.querySelector('#result h3').textContent = "Please decrease downpayment.";
        } else {
            downpaymentField.style.backgroundColor = '';
            document.querySelector("label[for='downpayment']").textContent = "Downpayment";
...
function setBankAttributes() {
    ...
    downpaymentSlider.max = currentBank.maxloan;
